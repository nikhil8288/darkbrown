...
CSV = os.path.join(os.path.dirname(__file__), "owner_rent_history.csv")
INV_TAG = "DB-HL-INV"
PAY_TAG = "DB-HL-PAY"
INV_RE = re.compile(r"\[(%s-\d{5})\]" % INV_TAG)

EXPECTED_ACCRUED = 3669500.00
EXPECTED_PAID = 3459500.00
...
def _checks(rows):
    ...
    paid_total = sum(flt(r.get("paid_amount")) for r in rows)
    ok = (abs(accrued - EXPECTED_ACCRUED) < 0.005
          and abs(paid_total - EXPECTED_PAID) < 0.005)
...
def run():
    ...
    for i, r in enumerate(rows):
        inv_no, pay_no = ("%s-%05d" % (INV_TAG, i + 1),
                          "%s-%05d" % (PAY_TAG, i + 1))
