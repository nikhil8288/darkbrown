...
INV_TAG = "DB-HIST-INV"
RCP_TAG = "DB-HIST-RCP"
INV_RE = re.compile(r"\[(%s-\d{5})\]" % INV_TAG)
...
EXPECTED_RENT = 5283008.00
EXPECTED_COLLECTED = 5158806.00
...
def run():
    ...
    cc_cache = {}
    def _cc(building):
        if building not in cc_cache:
            cc_cache[building] = L.cost_center(building)
        return cc_cache[building]
    missing = sorted({r["building"] for _, r, _ in resolved if not _cc(r["building"])})
    if missing:
        print("  ABORTING: no cost centre on %d building(s): %s"
              % (len(missing), ", ".join(missing)))
        print("  Every building gets one from the after_insert hook. A blank one")
        print("  means the hook could not find a group cost centre for the company.")
        return {"invoices": 0, "receipts": 0, "aborted": True}
    print("  cost centres   : %d resolved, one per building" % len(cc_cache))
    inv_seen, rcp_seen = _existing()
    ...
    for i, r, cust in resolved:
        ...
        if amount:
            if inv_no in inv_seen:
                ...
                si.due_date = due
                si.debit_to = debtor
                si.cost_center = _cc(r["building"])
                si.custom_billing_period = r["month"][:7]
                si.remarks = "[%s] | DB_HISTORY | %s %s | %s | %s" % (
