...
(function () {
  ...
  function getRedirectTarget(serverHomePage) {
    // 1) explicit ?redirect-to= wins, 2) server-decided home page, 3) site root
    const params = new URLSearchParams(window.location.search);
    return params.get("redirect-to") || serverHomePage || "/";
  }
  ...
  form.addEventListener("submit", function (e) {
    ...
    fetch("/api/method/login", {
      ...
      .then(function (response) {
        if (response.ok) {
          // The login API returns the server-decided landing page
          // (set per-role by darkbrown.auth.on_session_creation)
          return response.json().then(function (data) {
            window.location.href = getRedirectTarget(data && data.home_page);
          }).catch(function () {
            window.location.href = getRedirectTarget(null);
          });
        }
        return response.json().then(function (data) {
          const message =
