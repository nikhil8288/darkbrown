...
makes arrears real: the outstanding balance is what was charged minus what was
paid, not a number typed into a seed file.

WHY THE INVOICES ARE OPENING INVOICES

`is_opening = "Yes"` posts the debit against Temporary Opening rather than a
revenue account. These nine months already sit in the manual books; posting
them as income would count the same rent twice, once in Historical Monthly PL
and once in the ledger. The receivable and the receipt are both real, the
income is not re-recognised. This is the same treatment `seed_opening_arrears`
uses, applied to the whole history rather than to the residue.

WHY IT REPLACES THE ARREARS SEED

`seed_opening_arrears` existed to inject a closing arrears figure because no
transaction history was being loaded. With the history loaded the arrears fall
out of the ledger on their own — G-01B is charged 3,200 and pays 3,000 three
times over, so 600 is outstanding without anybody asserting it. Running both
would double the arrears. `opening_arrears.csv` ships empty for this reason.

HOW "COLLECTED" IS DERIVED

The Revenue sheet carries four money columns per month: Rent, Received,
Advance, Rent Due and Previous Due Rcvd. Cash actually in hand for a month is

    collected = Received + Advance - Previous Due Rcvd

Advance is money received, just paid ahead of its month. Previous Due Rcvd is
negative when an earlier shortfall was subsequently recovered, so subtracting it
adds that recovery back. Rent minus collected then reproduces the workbook's own
Net Rent Due exactly — 600.00 across the building, all of it G-01B — which is
the check the control totals below enforce.

TENANT ATTRIBUTION IS PER ROW, NOT PER UNIT

G-01A changed hands: Muhammed Ashique Paracholakuzhi to Dec-2025 at 3,400, the
unit empty through Jan and Feb, Mohammed Saeed Abdul Azeez from Mar-2026 at
3,000. The tenant named on each row owns that row's invoice and receipt.
Keying history by unit instead would credit one tenant's payments to another.

IDEMPOTENCY

Every invoice carries `[AK12-HIST-INV-nnn]` in its remarks and every receipt
carries `AK12-HIST-RCP-nnn` as its reference number, both zero-padded and
delimited so no tag is a prefix of another. A re-run after a partial failure
skips exactly what it created. Rows are read in a fixed sort order, so the
numbering is stable across runs.

Bench-execute only, never a Frappe patch: this posts to the ledger and a patch
runs unattended during a deploy.
"""
import csv
import os
import re
...
#: Straight off the workbook's own Reconciliation sheet. run() aborts on a
#: mismatch rather than posting a ledger that does not tie to the source.
...
def _resolve(rows, idx):
    """Exact normalised match only. No fuzzy fallback — tenants here share
    name tokens, and a token-overlap match posts one tenant's rent history
    onto another tenant's ledger."""
    resolved, problems = [], []
...
def _existing():
    """Tags already on the site, read once and compared as exact strings."""
    inv = set()
...
def dry_run():
    ...
    print("=" * 76)
    print("  rows %d | resolved %d | PROBLEMS %d" % (len(rows), len(resolved),
                                                     len(problems)))
    ...
    if problems:
        ...
        print("\n  No problems. run() would post %d invoices and %d receipts."
              % (len([1 for i, r, c in resolved
                 ...
                 len([1 for i, r, c in resolved
                      if _receipt_no(i) not in rcp_seen and flt(r["collected"])])))
    return {"rows": len(rows), "problems": len(problems),
            "rent": rent, "collected": coll}
...
def run():
    ...
    rent = sum(flt(r["rent"]) for r in rows)
    coll = sum(flt(r["collected"]) for r in rows)
    if (abs(rent - EXPECTED_RENT) > 0.005
            or abs(coll - EXPECTED_COLLECTED) > 0.005):
        ...
        if amount:
            if inv_no in inv_seen:
                ...
                si_name = frappe.db.get_value(
                    "Sales Invoice", {"remarks": ["like", "%[" + inv_no + "]%"],
                                      "docstatus": 1}, "name")
            else:
    ...
    print("  charged %s, collected %s, outstanding %s"
          % (format(rent, ",.2f"), format(coll, ",.2f"),
             format(rent - coll, ",.2f")))
    print("  Collection Cases open on the next auto_open_cases run.")
