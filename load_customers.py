...
def dry_run():
    ...
    unflagged = [n for n in have
                 if not any(frappe.db.get_value("Customer", c, "db_is_tenant")
                            for c in idx[_norm(n)])]
    print(f"  {len(have)} already exist as a Customer "
          f"({len(unflagged)} of them not yet flagged as a tenant - run() "
          f"will switch the flag on)")
...
def run():
    ...
    for n in names:
        if _norm(n) in idx:
            skipped += 1
            # A Customer that already exists — from an earlier load, or typed
            # into the desk — is still a tenant. Skipping it outright left the
            # flag off, and the Tenants screen filters on that flag, so the
            # tenant was invisible in the app even though the record and its
            # agreements were both fine. This mirrors what
            # `agreements._tenant()` does when it finds an existing party.
    ...
    print(f"  created {made}, skipped {skipped} "
          f"(of which {flagged} had the tenant flag switched on), "
          f"failed {len(failed)}")
    if failed:
